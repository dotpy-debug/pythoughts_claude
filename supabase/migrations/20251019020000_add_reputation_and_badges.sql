/*
  ...
  ### user_reputation
  Tracks reputation points and level for each user
  - `user_id` (uuid, primary key, references profiles)
  - `reputation_points` (integer) - Total reputation score
  - `level` (integer) - User level (calculated from points)
  - `posts_created` (integer) - Count of published posts
  - `comments_made` (integer) - Count of comments
  - `upvotes_received` (integer) - Total upvotes on content
  - `helpful_count` (integer) - Times marked as helpful
  ...
  - `category` (text) - Badge category (engagement, quality, milestone)
  - `color` (text) - Hex color for badge
  - `requirement_type` (text) - What triggers the badge
  ...
  - `is_featured` (boolean) - Display on profile
  ...
  - Post clapped: +1 point per clap
  - Post bookmarked: +3 points

  ## Level System
  - Level 1: 0-99 points (Newcomer)
  - Level 2: 100-499 points (Contributor)
  - Level 3: 500-1499 points (Regular)
  - Level 4: 1500-4999 points (Expert)
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'posts' AND TG_OP = 'INSERT' THEN
    ...
    points_change := 5; -- Post created

  ELSIF TG_TABLE_NAME = 'comments' AND TG_OP = 'INSERT' THEN
    target_user_id := NEW.author_id;
    points_change := 2; -- Comment made
  ...
  ELSIF TG_TABLE_NAME = 'bookmarks' AND TG_OP = 'INSERT' THEN
    SELECT author_id INTO target_user_id FROM posts WHERE id = NEW.post_id;
    points_change := 3; -- Content bookmarked
  END IF;
...
BEGIN
  ...
  LOOP
    -- Get the user's current value for this requirement
...
SELECT
  p.id,
  COALESCE(
    (SELECT COUNT(*) * 5 FROM posts WHERE author_id = p.id AND is_published = true) +
    (SELECT COUNT(*) * 2 FROM comments WHERE author_id = p.id) +
    (SELECT COALESCE(SUM(CASE WHEN vote_type = 1 THEN 10 ELSE -2 END), 0)
     FROM votes v
     JOIN posts po ON v.post_id = po.id
     WHERE po.author_id = p.id),
    0
  ) as points,
  calculate_user_level(COALESCE(
    (SELECT COUNT(*) * 5 FROM posts WHERE author_id = p.id AND is_published = true) +
    (SELECT COUNT(*) * 2 FROM comments WHERE author_id = p.id),
    0
  ))

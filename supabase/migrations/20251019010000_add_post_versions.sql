/*
  ...
  - `post_id` (uuid, references posts) - Post this version belongs to
  - `version_number` (integer) - Sequential version number (1, 2, 3...)
  - `title` (text) - Post title at this version
  - `content` (text) - Post content at this version
  - `subtitle` (text) - Post subtitle at this version
  - `image_url` (text) - Featured image URL at this version
  - `category` (text) - Category at this version
  - `changed_by` (uuid, references profiles) - User who created this version
  - `change_description` (text) - Optional description of changes
  - `is_major_edit` (boolean) - Flag for significant changes
  - `created_at` (timestamptz) - Version creation timestamp

  ## Features
  - Automatic version creation on post updates
  - Version comparison and diff tracking
  - Restore capability
  - Change attribution

  ## Security
  - Enable RLS on post_versions table
  - Post authors can view their own post versions
  - Public posts: anyone can view versions (for transparency)
  - Only post authors can create versions (via trigger)

  ## Indexes
  - Index on post_id and version_number for efficient lookups
...
-- Post authors can view all versions of their posts
...
-- This prevents manual version creation
...
BEGIN
  ...
  -- This will trigger the versioning system to create a new version of the current state
  UPDATE posts
  SET
...
-- This creates version 1 for all published posts
INSERT INTO post_versions (
  post_id,
  version_number,
  title,
